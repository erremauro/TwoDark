...
Table.print = function(obj, format, cb) {
  ...
  format = typeof format === 'function'
    ? format
    : function(leObj, cell) {
      for (var key in leObj) {
        if (!leObj.hasOwnProperty(key)) { continue }
        var params = opts[key] || {}
        cell(params.name || key, leObj[key], params.printer)
      }
    }
  ...
  if (Array.isArray(obj)) {
    cb = cb || function(tt) { return tt.toString() }
    obj.forEach(function(item) {
      format(item, cell)
      t.newRow()
    })
  } else {
    cb = cb || function(tt) { return tt.printTransposed({separator: ' : '}) }
    format(obj, cell)
    t.newRow()
  }
